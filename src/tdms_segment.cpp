        ...
        segment* previous_segment)
{
    ...
    for(size_t i = 0; i < num_objs; ++i)
    {
        ...
        /* ÏÐÅÎÁÐÀÇÓÞ Â ÍÓÆÍÓÞ ÊÎÄÈÐÎÂÊÓ */
        WCHAR buff[100];
        memset(buff, 0, 100);
        MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, object_path.c_str(), strlen(object_path.c_str()), buff, 99);
        object_path = utf8_encode(buff);
        /* ÏÐÅÎÁÐÀÇÓÞ Â ÍÓÆÍÓÞ ÊÎÄÈÐÎÂÊÓ */
